...
"""
@Project ：TemporalVAE 
@File    ：TemporalVAE_humanEmbryo_ref6DatasetAndKfoldXiang_onOneTimePointXiang.py
@Author  ：awa121
@Date    ：2025/3/21 8:05
...
# umap space is trained by 6 base ddataset
# query: one timepoints cell of Xiang, reference: other timepoints cells and 6 base dataset
def main():
    parser = argparse.ArgumentParser(description="TemporalVAE")
    parser.add_argument('--result_save_path', type=str,  # 2023-07-13 17:40:22
                        default="/Fig4_TemporalVAE_humanEmbryo_ref6DatasetAndKfoldXiangAndTyser_onOneTimePointXiang_epoch95/",
    ...
    #  ---------------------- Step1: use 6 dataset to train a umap space of reference  ------------------------
    drop_out_donor = ["T","Xiang"]
    print(f"drop the dataset: {drop_out_donor}")
    ...
    predict_donors_df['predicted_time'] = predict_donors_df['pseudotime'].apply(denormalize, args=(min(label_dic.keys()) / 100,
    ...
    #  ---------------------- Step2: use reference data (6 base + other timepoint of Xiang + Tyser) to train a model  ------------------------

    # test_donor_list = ['D6.0_xiang19', 'D7.0_xiang19']
    test_donor_list = ['D6.0_xiang19', 'D7.0_xiang19', 'D10.0_xiang19', 'D9.0_xiang19', 'D12.0_xiang19',  'D14.0_xiang19', 'D8.0_xiang19']
    # test_donor_list = ['D6.0_xiang19', 'D7.0_xiang19', 'D10.0_xiang19', 'D9.0_xiang19', 'D12.0_xiang19', 'D13.5_xiang19', 'D14.0_xiang19', 'D8.0_xiang19']
    ...
    adata_list = []
    for _test in test_donor_list:
        _mu_result = kFold_result_recall_dic[_test][-1]

        _, _test_mu_result = _mu_result
        _cell_time = cell_time.loc[predict_donors_dic[_test].index]
        _cell_time["predicted_time"] = predict_donors_dic[_test]['pseudotime'].apply(denormalize, args=(min(label_dic.keys()) / 100, max(label_dic.keys()) / 100,
                                                                                                        min(label_dic.values()), max(label_dic.values())))
        _cell_time = _cell_time[["time", "predicted_time", "dataset_label", "day", "cell_type"]]
        _adata_mu = ad.AnnData(X=_test_mu_result.cpu().numpy(), obs=_cell_time)
        _adata_mu.obs['data_type'] = 'Xiang'
        adata_list.append(_adata_mu)
    adata_mu_query = sc.concat(adata_list)  # 使索引唯一
    # -----------

    # ------------Step3: map query to ref umap space.
    ...
    queryOneDataset_referenceOn6Datasets_humanEmbryo("Xiang",
                                                     ...
                                                     adata_mu_query=adata_mu_query,
                                                     with_runner_predict_bool=False,
                                                     )
...
def queryOneDataset_referenceOn6Datasets_humanEmbryo(test_donor,
                                                     ...
                                                     umap_reducer,
                                                     special_file_name='',
                                                     with_runner_predict_bool=True,
                                                     adata_mu_query=None
                                                     ):
    # ---- set query data and predict on query data
    if with_runner_predict_bool:
        print(f"Predict with the runner:")
    ...
    # ----
    elif (with_runner_predict_bool is False) & (adata_mu_query is None):
        print(f"Error:without runner predicting and low-dimensional of query is None.")
        exit(1)
        return
