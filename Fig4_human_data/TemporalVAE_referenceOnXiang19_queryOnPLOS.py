...
"""
@Project ：TemporalVAE 
@File    ：TemporalVAE_referenceOnXiang19_queryOnPLOS.py
@IDE     ：PyCharm 
@Author  ：awa121
...
def main():
    save_path = f"results/Fig4_TemporalVAE_human_referenceXiang19_queryOnPLOS_240901/"
...
def directly_predict_on_vae(query_adata, save_path, checkpoint_file, config):
    ...
    color_dic = plot_violin_240223(query_adata.obs, save_path, x_attr="time", y_attr="predicted_time", special_file_name=method, color_map="turbo")
    # print(color_dic)
...
def train_LR_model(atlas_sc_expression_df, cell_time_atlas, query_adata, save_path, ):
    ...
    query_result_df.to_csv(f'{save_path}/{method}_result_df.csv', index=True)
    print(f"test result save at {save_path}/{method}_result_df.csv")
    color_dic = plot_violin_240223(query_result_df, save_path, x_attr="time", y_attr="pseudotime", special_file_name=method)
    plot_psupertime_density(query_result_df, save_path=save_path, label_key="time", psupertime_key="pseudotime", method=method)
