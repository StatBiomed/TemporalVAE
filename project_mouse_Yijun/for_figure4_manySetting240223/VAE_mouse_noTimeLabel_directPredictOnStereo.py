...
"""
@Project ：TemporalVAE 
@File    ：VAE_mouse_noTimeLabel_directPredictOnStereo.py
@IDE     ：PyCharm 
@Author  ：awa121
...
2024-02-23 12:49:08
for setting:
pre-trained model by atlas, directly predict on stereo data
...
def main():
    ...
    parser.add_argument('--result_save_path', type=str,  # 2023-07-13 17:40:22
                        default="240417_Figure4_manySetting/noTimeLabel_directPredictOnStereo/",
                        # default="240225_Figure4_manySetting/noTimeLabel_directPredictOnStereo/",
    ...
    # 计算每个细胞类型的数量
    celltype_counts = adata_stereo.obs["celltype_update"].value_counts()
    print(f"plot for {len(adata_stereo)} cells")
    if len(celltype_counts) > 10:
        # 选出数量前10的细胞类型
        top_10_attr = celltype_counts.head(10).index.tolist()
        # 根据选出的前10细胞类型筛选adata
        adata_stereo = adata_stereo[adata_stereo.obs["celltype_update"].isin(top_10_attr)].copy()
        print(f"plot for celltype_update, as the number is more than 10, select top 10 to plot umap: {top_10_attr}, the cell is filtered to {len(adata_stereo)}")
        cell_time_stereo = cell_time_stereo[cell_time_stereo["celltype_update"].isin(top_10_attr)]
        cell_time_stereo = cell_time_stereo.loc[adata_stereo.obs.index]
