...
"""
@Project ：TemporalVAE 
@File    ：VAE_mouse_kFoldOnStereoAndAtlas_withOrWithoutAdversarial.py
@IDE     ：PyCharm 
@Author  ：awa121
        ...
        with multiprocessing.Pool(processes=args.works) as pool:
            processed_results = pool.starmap(process_fold, [
                (fold, donor_list, sc_expression_df, donor_dic, batch_dic, special_path_str, cell_time,
                 time_standard_type, config, args, 100000, args.adversarial_train_bool) for fold in range(len(donor_list))])
        try:
            ...
            while True:
                ...
                with multiprocessing.Pool(processes=args.works) as pool:
                    processed_results = pool.starmap(process_fold, [
                        (fold, donor_list, sc_expression_df, donor_dic, batch_dic, special_path_str, cell_time,
                         time_standard_type, config, args, 100000, args.adversarial_train_bool) for fold in re_fold_list])
                #2023-09-20 00:02:01
