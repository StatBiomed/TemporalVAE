...
source ~/.bashrc
nohup python -u project_mouse_Yijun/VAE_mouse_fineTune_Train_on_U_pairs_S.py > logs/VAE_mouse_fineTune_unet.log 2>&1 &
...
def main():
    ...
    parser.add_argument('--sc_file_name', type=str,
                        ...
                        # default="240108mouse_embryogenesis/hematopoiesis_atlasGene_hvgs1704_hbby",
    ...
    if "E16" in args.sc_file_name:
        sc_file_name = f"/mnt/yijun/public/for_yijun_fangxin/{args.sc_file_name}.h5ad"
    else:
        sc_file_name = f"/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/data/{args.sc_file_name}.h5ad"
    ...
    mouse_atlas_file = "/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/data/" \
                       ...
                       "data_count_hvg.csv"
    config_file = "/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/vae_model_configs/" \
                  "supervise_vae_regressionclfdecoder_mouse_atlas_finetune_u_s_focusEncoder.yaml"
    # config_file = "/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/vae_model_configs/" \
    #               "supervise_vae_regressionclfdecoder_mouse_atlas_finetune_u_s_focusEncoder_moreFeatures.yaml"
    ...
    save_result_path = f"/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/results/" \
                       f"{args.special_path_str}/{_temp}/{fine_tune_mode}_clfWeight{clf_weight}_detT{args.detT}/"
    ...
    # ----------------- read test adata -----------------
    if "fangxin" in sc_file_name:
        adata_concated, adata, gene_dic, hvg_dic = read_fangxinData(sc_file_name)
    elif "atlasGene_hvgs" in sc_file_name:
        adata_concated, adata, gene_dic, hvg_dic = read_mouse_embryogenesisData_atlasGene_hvgs(sc_file_name)
    else:
...
def read_mouse_embryogenesisData_atlasGene_hvgs(sc_file_name):
    #
    _temp = "/mnt/yijun/nfs_share/awa_project/pairsRegulatePrediction/GPLVM_dandan/data/240108mouse_embryogenesis/"
    ...
    hvg_dic = list(set(adata.var[adata.var["is_atlasGenes"] == 0].index))
...
def read_fangxinData(sc_file_name):
    ...
    # 2024-01-11 10:17:26 for new dataset
    ...
    def temp_process_day(day_str):
        return int(day_str.replace('E', ''))

    # Apply the custom function to create the "time_label" column
    adata.obs["time_label"] = adata.obs["age"].apply(temp_process_day)
    adata.obs['cell_type_encoded'] = adata.obs['cell_type'].astype('category').cat.codes
    # Note-- unsplice data is more sparse than splice data
    adata_unspliced = adata.copy()
    adata_unspliced.X = adata.layers["Mu"]
