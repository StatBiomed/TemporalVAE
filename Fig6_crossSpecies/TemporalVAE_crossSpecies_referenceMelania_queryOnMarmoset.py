...
"""
@Project ：TemporalVAE 
@File    ：TemporalVAE_crossSpecies_referenceMelania_queryOnMarmoset.py.py
@IDE     ：PyCharm 
@Author  ：awa121
...
def main():
    ...
    if args.train_whole_model:
        cell_drop_index_list = cell_time.loc[cell_time["species"] == "marmoset"].index
    ...
    if args.kfold_test:
        ...
        cell_time_query = cell_time_query[["time", "predicted_time", "dataset_label", "Stage", "species", "Lab"]]
        ...
        # adata_all.obs["cell_typeMask4dataset"] = adata_all.obs.apply(lambda row: 'l & m & p & z & xiao' if row['species'] != 'marmoset' else row['cell_type'], axis=1)
        ...
        plot_tyser_mapping_to_4dataset_predictedTime(adata_all.copy(), save_file_name, label_dic,
                                                     mask_dataset_label='marmoset', plot_attr='predicted_time', mask_str="species",
                                                     reference_dataset_str=reference_dataset_str,
                                                     special_file_str="_maskMarmoset"
                                                     )
        plot_tyser_mapping_to_4dataset_predictedTime(adata_all.copy(), save_file_name, label_dic,
        ...
        plot_tyser_mapping_to_4dataset_predictedTime(adata_all.copy(), save_file_name, label_dic,
                                                     mask_dataset_label='marmoset', plot_attr='time', mask_str="species",
                                                     reference_dataset_str=reference_dataset_str,
                                                     special_file_str="_maskMarmoset"
                                                     )
        plot_tyser_mapping_to_4dataset_predictedTime(adata_all.copy(), save_file_name, label_dic,
