...
def main():
    ...
    dataset_list = ["acinarHVG", "humanGermline", "embryoBeta", "acinarHVG"]
    ...
    for dataset in dataset_list:
        ...
        # 初始化标签列表
        all_labels = list(np.sort(np.unique(pca_df["time"]))) + ['No removal']
        plot_kFold_corr(pca_df.copy(), randomForest_df.copy(), psupertime_df.copy(), vae_df.copy(), dataset, all_labels, dataset_dic)
        plot_boxAndDot_on_allData(pca_df.copy(), randomForest_df.copy(), psupertime_df.copy(), vae_df.copy(), dataset, dataset_dic)
...
def plot_boxAndDot_on_allData(pca_df, randomForest_df, psupertime_df, vae_df, dataset, dataset_dic):
    # 初始化相关系数列表
    ...
    df_concat = pd.concat([pca_df, randomForest_df, psupertime_df, vae_df], keys=['PCA', 'RF', 'Psupertime', 'TemporalVAE'])
    ...
    g = sns.FacetGrid(df_concat.reset_index(level=0), col='level_0', col_wrap=2, sharex=True, sharey=True,
                      aspect=label_num * 0.2, height=3.5)
...
def plot_kFold_corr(pca_df, randomForest_df, psupertime_df, vae_df, dataset, all_labels, dataset_dic):
    plt.close()
    # 初始化相关系数列表
    ...
    for label_to_remove in all_labels:
        ...
        randomForest_spearman_correlations.append(
            calculate_corr(randomForest_df, label_to_remove, corr_method="spearmanr"))
        ...
        randomForest_kendalltau_correlations.append(
            calculate_corr(randomForest_df, label_to_remove, corr_method="kendalltau"))
    ...
    fig, ax1 = plt.subplots(1, 1, figsize=(len(all_labels), 4), sharex=False)
    # fig, ax1 = plt.subplot(figsize=(len(all_labels), 4), sharex=False)
    # fig, (ax1, ax2) = plt.subplots(1, 1, figsize=(10, 8), sharex=True)

    # 子图1：Spearman相关性
    ax1.bar(x - 1.5 * width, pca_spearman_correlations, width, label='PCA', color="#ECEE81")
    ax1.bar(x - 0.5 * width, randomForest_spearman_correlations, width, label='RF', color="#8DDFCB")
    ax1.bar(x + 0.5 * width, psupertime_spearman_correlations, width, label='Psupertime', color="#82A0D8")
    ax1.bar(x + 1.5 * width, vae_spearman_correlations, width, label='TemporalVAE', color="#EDB7ED")
